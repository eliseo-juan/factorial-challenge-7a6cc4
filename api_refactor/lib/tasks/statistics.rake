namespace :statistics do
  ...
  task calculate: :environment do
    ...
    Question.includes(:answers).find_each do |q|
    ...
    puts "Average number of tags per question: #{tags_per_question}"
    puts "Average number of answers per question: #{answers_per_question}"
    puts "Top 5 users by questions answered: #{top_5_users.map(&:username)}"
    puts "Answer rating per tag count: #{answer_rating_per_tag_count}"
